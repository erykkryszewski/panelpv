...
if (!class_exists('TGM_Plugin_Activation')) {
    ...
    class TGM_Plugin_Activation
    {
            ...
            foreach ($this->plugins as $slug => $plugin) {
                if (false === $this->can_plugin_activate($slug)) {
                    add_filter(
                        ...
                        [$this, 'filter_plugin_action_links_activate'],
                        20,
                    );
                }
                ...
                if (true === $plugin['force_activation']) {
                    add_filter(
                        'plugin_action_links_' . $plugin['file_path'],
                        [$this, 'filter_plugin_action_links_deactivate'],
                        20,
                    );
                }
                ...
                if (false !== $this->does_plugin_require_update($slug)) {
                    add_filter(
                        'plugin_action_links_' . $plugin['file_path'],
                        [$this, 'filter_plugin_action_links_update'],
                        20,
                    );
                }
            }
        ...
        public function install_plugins_page()
        {
            ...
            if (
                (('tgmpa-bulk-install' === $plugin_table->current_action() ||
                    'tgmpa-bulk-update' === $plugin_table->current_action()) &&
        ...
        protected function do_plugin_install()
        {
            ...
            if (
                (isset($_GET['tgmpa-install']) && 'install-plugin' === $_GET['tgmpa-install']) ||
                    ...
                    echo '<p>',
                        sprintf(
                            ...
                            '<a href="' .
                                ...
                                '">' .
                                esc_html($this->strings['dashboard']) .
                                '</a>',
                        ),
                        '</p>';
                ...
                isset($this->plugins[$slug]['file_path'], $_GET['tgmpa-activate']) &&
        ...
        public function maybe_adjust_source_dir($source, $remote_source, $upgrader)
        {
            ...
            if (false === $upgrader->bulk && !empty($upgrader->skin->options['extra']['slug'])) {
                ...
                foreach ($this->plugins as $slug => $plugin) {
                    if (
                        !empty($upgrader->skin->plugin_names[$upgrader->skin->i]) &&
                    ...
                    if (!$automatic) {
                        ...
                        if (!isset($_POST['action'])) {
                            echo '<div id="message" class="updated"><p>',
                                esc_html($this->strings['activated_successfully']),
                                ' <strong>',
        ...
        public function notices()
        {
            ...
            if (
                $this->is_tgmpa_page() ||
                $this->is_core_update_page() ||
                get_user_meta(get_current_user_id(), 'tgmpa_dismissed_notice_' . $this->id, true) ||
            ...
            $action_links = [
                ...
                'dismiss' => $this->dismissable
                    ? '<a href="' .
                        ...
                        '" class="dismiss-notice" target="_parent">' .
        ...
        {
            if (
                !empty($this->plugins[$slug]['external_url']) &&
        ...
        {
            return (!empty($this->plugins[$slug]['is_callable']) &&
                is_callable($this->plugins[$slug]['is_callable'])) ||
    ...
    class TGMPA_List_Table extends WP_List_Table
    {
        ...
        public function __construct()
        {
            ...
            if (
                ...
                in_array($_REQUEST['plugin_status'], ['install', 'update', 'activate'], true)
            ) {
        ...
        protected function get_plugin_status_text($slug)
        {
            ...
            if (
                $this->tgmpa->does_plugin_require_update($slug) &&
        ...
        public function column_version($item)
        {
            ...
            if ($this->tgmpa->is_plugin_installed($item['slug'])) {
                $installed = !empty($item['installed_version'])
                    ? $item['installed_version']
                    : _x('unknown', 'as in: "version nr unknown"', 'tgmpa');
                ...
                $output[] = sprintf(
                    '<p><span style="min-width: 32px; text-align: right; float: right;%1$s">%2$s</span>' .
                        __('Installed version:', 'tgmpa') .
                        '</p>',
                    $color,
                    $installed,
                );
            ...
            if (!empty($item['minimum_version'])) {
                $output[] = sprintf(
                    '<p><span style="min-width: 32px; text-align: right; float: right;">%1$s</span>' .
                        __('Minimum required version:', 'tgmpa') .
                        '</p>',
            ...
            if (!empty($item['available_version'])) {
                $color = '';
                if (
                    !empty($item['minimum_version']) &&
                    version_compare($item['available_version'], $item['minimum_version'], '>=')
                ) {
        ...
        {
            echo esc_html__('No plugins to install, update or activate.', 'tgmpa') .
                ' <a href="' .
                esc_url(self_admin_url()) .
                '"> ' .
        ...
        protected function get_row_actions($item)
        {
            ...
            if (!$this->tgmpa->is_plugin_installed($item['slug'])) {
                ...
                if (
                    false !== $this->tgmpa->does_plugin_have_update($item['slug']) &&
                    $this->tgmpa->can_plugin_update($item['slug'])
                ) {
                    /* translators: %2$s: plugin name in screen reader markup */
        ...
        public function process_bulk_actions()
        {
            ...
            if ('tgmpa-bulk-install' === $this->current_action() || 'tgmpa-bulk-update' === $this->current_action()) {
                ...
                echo '<div class="tgmpa">',
                    '<h2 style="font-size: 23px; font-weight: 400; line-height: 29px; margin: 0; padding: 9px 15px 4px 0;">',
            ...
            if (!class_exists('TGMPA_Bulk_Installer')) {
                ...
                class TGMPA_Bulk_Installer extends Plugin_Upgrader
                {
                    ...
                    public function __construct($skin = null)
                    {
                        ...
                        if (
                            isset($this->skin->options['install_type']) &&
                ...
                class TGMPA_Bulk_Installer_Skin extends Bulk_Upgrader_Skin
                {
                    ...
                    public function bulk_footer()
                    {
                        ...
                        if ($this->tgmpa->is_tgmpa_complete()) {
                            ...
                            $update_actions['tgmpa_page'] =
                                '<a href="' .
                                esc_url($this->tgmpa->get_tgmpa_url()) .
                                '" target="_parent">' .
...
if (!class_exists('TGMPA_Utils')) {
    ...
    class TGMPA_Utils
    {
        ...
        protected static function emulate_filter_bool($value)
        {
            ...
            static $true = ['1', 'true', 'True', 'TRUE', 'y', 'Y', 'yes', 'Yes', 'YES', 'on', 'On', 'ON'];
            static $false = ['0', 'false', 'False', 'FALSE', 'n', 'N', 'no', 'No', 'NO', 'off', 'Off', 'OFF'];
            // phpcs:enable
