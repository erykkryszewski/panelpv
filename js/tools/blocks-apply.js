...
function rewriteStyleScss(stylePath, scssBlocksDir, keep, scssAliases, removeWoo) {
  ...
  if (removeWoo) {
    const wooRe = /^\s*@import\s+(?:url\((?:'|")?)?scss\/woocommerce\/[^'")]+(?:'|")?\)?\s*;\s*$/gim;
  ...
  const replaced = replaceBetweenMarkers(txt, '/* @blocks:start */', '/* @blocks:end */', imports);
  if (replaced !== null) {
  ...
  const blocksLineRe = /^\s*(?:\/\/\s*)?@import\s+(?:url\((?:'|")?)?scss\/blocks\/[^'")]+(?:'|")?\)?\s*;\s*$/gim;
